...
function TravelModule:CreateFrames()
    ...
    -- Housing Part
    self.houseButton = self.houseButton or
                            CreateFrame('BUTTON', 'houseButton',
                                        self.hearthFrame,
                                        'SecureActionButtonTemplate')
    self.houseIcon = self.houseIcon or
                          self.houseButton:CreateTexture(nil, 'OVERLAY')
...
function TravelModule:RegisterFrameEvents()
    ...
    self.houseButton:EnableMouse(true)
    self.houseButton:RegisterForClicks('AnyUp', 'AnyDown')
    self.houseButton:SetScript('OnClick', function()
        if C_Housing and C_Housing.EnterHouse then
            C_Housing.EnterHouse()
        else
            print("|cFF00FFFFXIV_Databar:|r L'API Housing n'est pas encore disponible.")
        end
    end)

    self.houseButton:SetScript('OnEnter', function(self)
        if InCombatLockdown() then return end
        GameTooltip:SetOwner(self, 'ANCHOR_' .. xb.miniTextPosition)
        GameTooltip:ClearLines()
        GameTooltip:AddLine("Maison")
        GameTooltip:AddLine("Téléportation vers votre foyer", 1, 1, 1)
        GameTooltip:Show()
        TravelModule.houseIcon:SetVertexColor(unpack(xb:HoverColors()))
    end)

    self.houseButton:SetScript('OnLeave', function()
        GameTooltip:Hide()
        TravelModule.houseIcon:SetVertexColor(xb:GetColor('normal'))
    end)
...
function TravelModule:Refresh()
    ...
    -- Housing Part
    self.houseButton:SetSize(iconSize, xb:GetHeight())
    self.houseButton:SetPoint("RIGHT", self.hearthButton, "LEFT", -(db.general.barPadding), 0)

    self.houseIcon:SetTexture(xb.constants.mediaPath .. 'datatexts\\house')
    self.houseIcon:SetSize(iconSize, iconSize)
    self.houseIcon:SetPoint("CENTER")
    self.houseIcon:SetVertexColor(xb:GetColor('normal'))
    self.houseButton:Show()
    ...
    self.portButton:SetSize(self.portText:GetWidth() + iconSize +
                                db.general.barPadding, xb:GetHeight())
    self.portButton:SetPoint("RIGHT", self.houseButton, "LEFT",
                             -(db.general.barPadding), 0)
    ...
    if self.houseButton:IsVisible() then
        totalWidth = totalWidth + self.houseButton:GetWidth() + db.general.barPadding
    end
